...
MODEL_SAVE_PATH = "cnn_pairwise_scorer.pt"
BEST_MODEL_SAVE_PATH = "cnn_pairwise_scorer_best.pt"
...
def main():
    ...
    # Define optimizer and Pairwise Ranking Loss (SoftMarginLoss)
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    # We want score_pos > score_neg, so loss is on (score_pos - score_neg) with target 1
    ranking_loss_fn = nn.SoftMarginLoss()
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{NUM_EPOCHS} [Train]"):
            ...
            # The model takes query and document as separate inputs
