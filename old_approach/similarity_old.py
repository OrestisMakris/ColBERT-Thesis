...
# def main_inference(index_name, output_dir, ...):
#     # 1) Build Searcher/Scorer
#     config = ColBERTConfig.load_from_index(index_name)
#     scorer = IndexScorer(index_name, use_gpu=torch.cuda.is_available())
#     scorer.set_embeddings_strided()

#     os.makedirs(output_dir, exist_ok=True)

#     # 2) For each query:
#     for q_idx, qtext in enumerate(queries_texts):
#         # a) encode query into ColBERT tensor [1, q_len, dim]
#         Q = scorer.checkpoint.queryFromText([qtext], bsize=1, to_cpu=False)

#         # b) retrieve candidate pids (e.g. top-K, or all)
#         pids, centroid_scores = scorer.retrieve(config, Q)

#         # c) lookup packed passage embeddings + lengths
#         D_packed, D_lengths = scorer.lookup_pids(pids)

#         # d) raw dot‑product sim: [total_tokens, q_len]
#         sim_flat = D_packed @ Q.squeeze(0).transpose(0,1)

#         # e) rebuild padded sim matrix: [num_pids, max_doc_len, q_len]
#         from colbert.search.strided_tensor import StridedTensor
#         sim_padded, sim_mask = StridedTensor(sim_flat, D_lengths, use_gpu=scorer.use_gpu).as_padded_tensor()

#         # f) re‑shape to [num_pids, q_len, max_doc_len]
#         sim_matrix = sim_padded.permute(0,2,1).cpu()

#         # g) export each (q,pid) matrix
#         for i, pid in enumerate(pids.tolist()):
#             path = f"{output_dir}/q{q_idx}_d{pid}.pt"
#             torch.save(sim_matrix[i], path)

#         # h) (optionally) still run your scorer to write out final ranking
#         scores, _ = scorer.score_pids(config, Q, pids, centroid_scores)
#         # … write top‑K scores/pids to your TSV

#     print_message(f"Exported Q×D matrices to {output_dir}")