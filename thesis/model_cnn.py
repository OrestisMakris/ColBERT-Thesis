...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        # Kernel: (15, 35), Padding: (2, 2)
        self.conv1 = nn.Conv2d(1, 16, kernel_size=(5, 15), padding=(1, 1))
        self.bn1   = nn.BatchNorm2d(16)
        
        # Second convolutional layer
        # Kernel: (10, 20), Padding: (2, 2)
        self.conv2 = nn.Conv2d(16, 16, kernel_size=(5, 5), padding=(1, 1))
        self.bn2   = nn.BatchNorm2d(16)
        
        self.pool  = nn.AvgPool2d(2) 
        self.dropout = nn.Dropout(0.2)
        
        # The input features to fc1 depend on the output size of conv2 and pooling.
        # This will be 16 (channels) if adaptive_avg_pool2d reduces spatial to 1x1.
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = F.selu(x)
        x = self.pool(x) # Dimensions halved

        # Layer 2
        x = self.conv2(x)
        x = self.bn2(x)
        x = F.selu(x)
        x = self.pool(x) # Dimensions halved again
